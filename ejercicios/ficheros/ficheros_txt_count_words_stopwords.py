# https://docs.python.org/3/tutorial/inputoutput.html#reading-and-writing-files
# https://docs.python.org/3/library/stdtypes.html#string-methods
# https://docs.python.org/3/library/pprint.html
# https://docs.python.org/3/library/re.html
# https://docs.python.org/3/howto/sorting.html
# https://www.nltk.org/
# https://www.clips.uantwerpen.be/pattern


import pprint
import re

def get_count(word):
    return word_counts[word]

def get_stopwords(file_name):

    stop_dict = {}

    my_file = open(file_name, "r")

    for line in my_file:
        word = line.rstrip()

        stop_dict[word] = 1

    return stop_dict

STOP_FILE = "data/en_stopwords.txt"
FILE_NAME = "data/alice_in_wonderland.txt"

FILE_OUT = "data/alice_counts_stop.csv"

# Load stopwords

my_stop = get_stopwords(STOP_FILE)

pprint.pprint(my_stop)

# Read file into my_lines list

my_file = open(FILE_NAME, "r")

my_lines = []

for line in my_file:
    my_lines.append(line.rstrip())

my_file.close()

word_counts = {}

# tokenize each line and count if not in stopwords

for line in my_lines:
    words = re.split('\W+', line)
    #words = line.split(" ")
    # pprint.pprint(words)
    for word in words:

        if len(word) > 2:
            word_lower = word.lower()

            if word_lower not in my_stop:
                if word_lower not in word_counts:
                    word_counts[word_lower] = 0
                word_counts[word_lower] = word_counts[word_lower] + 1

#pprint.pprint(word_counts)

# sort words

words_ordered = sorted(word_counts.keys(), key = get_count, reverse = True)

# write counts to csv

file_out = open(FILE_OUT, "w")
file_out.write("word,count\n")

for word in words_ordered:
    print(word + "-->", word_counts[word])
    file_out.write(word + "," + str(word_counts[word]) + "\n")


file_out.close()







